...
* <
	...
	lea     physique(pc),a0          ; Swapping of two Screens
	move.l	(a0),d0                  ;
	move.l	4(a0),(a0)+              ;
	move.l	d0,(a0)                  ;
...
Timer_Raster:
	...
	move.l  #Raster_palette,a0       ; Get palette of Raster
	add.l   #44*9,a0                 ; Add 44*9 words
.loop:
	move.b  (a1),d0                  ; Get value of timer B
.wait1:
	cmp.b   (a1),d0                  ; Wait one scanline 
	beq.s   .wait1                   ; Keep looping until 1 scanline reached
	move.w  -(a0),$ff8240            ; Set our colours backwards!
	dbra    d2,.loop                 ; Keep looping until all cols done

	movem.l	Palette_Image,d1-d7/a0   ; Get the palette of the picture
	move.b	(a1),d0                  ; Get the value of timer B
.wait2:
	cmp.b	(a1),d0                    ; Wait another scanline
	beq.s	.wait2                     ; Until we have 	
...
newtb2:
	...
	move.l a1,-(sp)                  ; Store register

	move.w  #$fa21,a1 
	move.b  #76,(a1)                 ; Our next interrupt 
	move.b  #8,$fffffa1b.w           ; Timer B

	move.l (sp)+,a1                  ; Restore old register
...
Save_and_init_st:
	...
	move.l  #Timer_Raster,$120       ; New timer B vector
...
.put_scrolling:
	...
	add.w	#160*180,a1                ; Add 180 lines to start at the end of the screen
	addq.w	#2,a1                    ; Add plane for the color
...
TEXTE:
	...
	dc.b	"@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz"
	dc.w	$FF,$0
	even
; Rasters
Raster_palette:
